...
# df_label = df['label_1']
# df_features = df.drop(['label_1'], axis=1)

# std_scaler = StandardScaler()
 
# df_scaled = std_scaler.fit_transform(df_features.to_numpy())
# df_scaled = pd.DataFrame(df_scaled, columns=[
#  'CreditScore', 'Age', 'Tenure', 'Balance', 'NumOfProducts', 'HasCrCard',
#        'IsActiveMember', 'EstimatedSalary', 'Germany', 'Spain', 'Male'])

# df = pd.concat([df_scaled, df_label], axis=1)
...
print(df_train['label_1'].value_counts())
...
print(classification_report(y_test, y_pred_test_5_KNN))
...
print(classification_report(y_test, y_pred_test_GBM_50))
...
print(classification_report(y_test, y_pred_test_GBM_100))
...
print(classification_report(y_test, y_pred_test_GBM_200))
...
print(classification_report(y_test, y_pred_test_RNF))
...
# model = Sequential()
# model.add(LSTM(units=128, input_shape=(1, X_train.shape[2]), return_sequences=True))
...
# print(classification_report(y_test, binary_preds_LSTM))
...
# model_GRU.add(GRU(units=128, input_shape=(1, X_train.shape[2]), return_sequences=True))
...
# score, accuracy = model_GRU.evaluate(x_test, y_test)
# print('Test score:', score)
# print('Test accuracy:', accuracy)
...
# y_pred_proba_GRU=model_GRU.predict(x_test)
# # set threshold for converting probabilities to binary classes
# threshold = 0.5

# # convert probabilities to binary classes using threshold
# binary_preds_GRU = np.where(y_pred_proba_GRU >= threshold, 1, 0)


# print(classification_report(y_test, binary_preds_GRU))

# In[110]:


# # predict_classes=np.argmax(y_pred_proba_LSTM,axis=1)
# df_actual_predicted_GRU = pd.concat([pd.DataFrame(np.array(y_test), columns=['label_1']), pd.DataFrame(y_pred_proba_GRU, columns=['y_pred_proba_GRU'])], axis=1)
# df_actual_predicted_GRU.index = y_test.index
# fpr, tpr, tr = roc_curve(df_actual_predicted_GRU['label_1'], df_actual_predicted_GRU['y_pred_proba_GRU'])
# auc = roc_auc_score(df_actual_predicted_GRU['label_1'], df_actual_predicted_GRU['y_pred_proba_GRU'])

# plt.plot(fpr, tpr, label='AUC = %0.4f' %auc)
# plt.plot(fpr, fpr, linestyle = '--', color='k')
# plt.xlabel('False Positive Rate')
# plt.ylabel('True Positive Rate')
# plt.title('ROC Curve for GRU', size = 10)
# plt.legend()
...
# model_RNN.add(SimpleRNN(units=128, input_shape=(1, X_train.shape[2]), return_sequences=True))
...
# score, accuracy = model_RNN.evaluate(x_test, y_test)
# print('Test score:', score)
# print('Test accuracy:', accuracy)
...
# # convert probabilities to binary classes using threshold
# binary_preds_RNN = np.where(y_pred_proba_RNN >= threshold, 1, 0)


# In[114]:


# print(classification_report(y_test, binary_preds_RNN))
...
# # predict_classes=np.argmax(y_pred_proba_LSTM,axis=1)
# df_actual_predicted_RNN = pd.concat([pd.DataFrame(np.array(y_test), columns=['label_1']), pd.DataFrame(y_pred_proba_RNN, columns=['y_pred_proba_RNN'])], axis=1)
# df_actual_predicted_RNN.index = y_test.index
# fpr, tpr, tr = roc_curve(df_actual_predicted_RNN['label_1'], df_actual_predicted_RNN['y_pred_proba_RNN'])
# auc = roc_auc_score(df_actual_predicted_RNN['label_1'], df_actual_predicted_RNN['y_pred_proba_RNN'])

# plt.plot(fpr, tpr, label='AUC = %0.4f' %auc)
# plt.plot(fpr, fpr, linestyle = '--', color='k')
# plt.xlabel('False Positive Rate')
# plt.ylabel('True Positive Rate')
# plt.title('ROC Curve for RNN', size = 10)
# plt.legend()
...
fig = plt.figure(linewidth=2, facecolor='snow')
fig.set_figwidth(16)
fig.set_figheight(9)
...
#######################################################################################
#fit GRU model and plot ROC curve ---->

# fpr, tpr, _ = metrics.roc_curve(df_actual_predicted_GRU['label_1'], df_actual_predicted_GRU['y_pred_proba_GRU'])
# auc = round(metrics.roc_auc_score(df_actual_predicted_GRU['label_1'], df_actual_predicted_GRU['y_pred_proba_GRU']), 4)
# plt.plot(fpr,tpr,label="GRU, AUC="+str(auc), linewidth=3)

#######################################################################################
#fit RNN model and plot ROC curve ---->

# fpr, tpr, _ = metrics.roc_curve(df_actual_predicted_RNN['label_1'], df_actual_predicted_RNN['y_pred_proba_RNN'])
# auc = round(metrics.roc_auc_score(df_actual_predicted_RNN['label_1'], df_actual_predicted_RNN['y_pred_proba_RNN']), 4)
# plt.plot(fpr,tpr,label="RNN, AUC="+str(auc), linewidth=3)
...
# add label
plt.xlabel('False Positive Rate', color='black', size=18)
plt.ylabel('True Positive Rate', color='black', size=18)

# add title
plt.title('ROC Curve for clssification models after Two-phase resampling - Telco dataset', size = 18)


#add legend
plt.grid(which='major', color='black', linestyle='--')
plt.legend(facecolor='gainsboro', framealpha=1, loc=0, prop={'size': 16})
...
fig.savefig('after_resampling_Usup_telco.png', dpi=300)
...
# # print('f1-score for KNN-5: {}'.format(f1_score(y_test,y_pred_test_5_KNN)))
# auc_LSTM_df = [['LSTM', auc_LSTM, f1_score_LSTM]]
# LSTM_reults = pd.DataFrame(auc_LSTM_df, columns=['Model name', 'AUC', 'f1_score'])


# f1_score_GRU = f1_score(y_test,binary_preds_GRU)
# auc_GRU = metrics.roc_auc_score(y_test, binary_preds_GRU)
# # print('f1-score for KNN-5: {}'.format(f1_score(y_test,y_pred_test_5_KNN)))
# auc_GRU_df = [['GRU', auc_GRU, f1_score_GRU]]
# GRU_reults = pd.DataFrame(auc_GRU_df, columns=['Model name', 'AUC', 'f1_score'])


# f1_score_RNN = f1_score(y_test, binary_preds_RNN)
# auc_RNN = metrics.roc_auc_score(y_test, binary_preds_RNN)
# # print('f1-score for KNN-5: {}'.format(f1_score(y_test,y_pred_test_5_KNN)))
# auc_RNN_df = [['RNN', auc_RNN, f1_score_RNN]]
# RNN_reults = pd.DataFrame(auc_RNN_df, columns=['Model name', 'AUC', 'f1_score'])
...
# frames = [CART_reults, KNN_1_reults, KNN_3_reults, KNN_5_reults, GBM_50_reults, GBM_100_reults,
#           GBM_200_reults, SVC_RBF_reults, SVC_Linear_results, XGBoost_reults, RNF_reults, CatBoost_reults,
#           LR_reults, LSTM_reults, GRU_reults, RNN_reults]
